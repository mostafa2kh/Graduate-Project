package com.rentsphere.media.exception;
...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<Map<String, Object>> handleValidation(MethodArgumentNotValidException ex) {
        List<Map<String, String>> details = ex.getBindingResult().getFieldErrors().stream().map(e -> {
            ...
            d.put("field", e.getField()); d.put("message", e.getDefaultMessage());
            return d;
        }).toList();
    ...
    public ResponseEntity<Map<String, Object>> handleSecurity(SecurityException ex) {
        return error(HttpStatus.FORBIDDEN, "ACCESS_DENIED", ex.getMessage(), List.of());
    }
